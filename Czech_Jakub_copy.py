...
class Project:
    ...
    def crop_elements(self):
        ...
        for img in self.images:
            ...
            ori_img = self.images[img][0]
            cnts = self.get_cnts(img_tmp, ori_img)
            for c in cnts:
    ...
    @staticmethod
    def get_cnts(img, ori_img):
        ori_img_gray = cv.cvtColor(ori_img, cv.COLOR_BGR2GRAY)
        gray = cv.medianBlur(ori_img_gray, 5)
        thresh = cv.adaptiveThreshold(gray, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 11, 2)
        thresh = thresh.astype(np.uint8)
        cnts_1 = cv.findContours(thresh, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        cnts_1 = grab_contours(cnts_1)
